...
void SPI_Master_Init( void) {
  ...
  //SPI 2 uses PD0 for NSS, PD1 fo r SCL, PD3 for MISO, PD4 for MOSI
  RCC->APB2ENR |= RCC_APB2ENR_SPI2EN;
  RCC->AHB2ENR |= (RCC_AHB2ENR_GPIODEN);

  GPIOS->MODER &= ~(GPIO_MODER_MODE12 | GPIO_MODER_MODE13 | GPIO_MODER_MODE15);
  // set MODE4/5/7 to 10 (alternate function)
  GPIOD->MODER |= (GPIO_MODER_MODE12_1 | GPIO_MODER_MODE13_1 | GPIO_MODER_MODE15_1);
  // push-pull (0)
  GPIOD->OTYPER &= ~(GPIO_OTYPER_OT12 | GPIO_OTYPER_OT13 | GPIO_OTYPER_OT15);
  // low speed (0)
  GPIOD->OSPEEDR &= ~(GPIO_OSPEEDR_OSPEED12 | GPIO_OSPEEDR_OSPEED13 | GPIO_OSPEEDR_OSPEED15);
  // no pull up / no pull down (00)
  GPIOD->PUPDR &= ~(GPIO_PUPDR_PUPD12 | GPIO_PUPDR_PUPD13 | GPIO_PUPDR_PUPD15);

  GPIOD->AFR[1] |= (5 << GPIO_AFRH_AFSEL12_Pos |
					  5 << GPIO_AFRH_AFSEL13_Pos | //SCL
					  5 << GPIO_AFRH_AFSEL14_Pos | //MISO
					  5 << GPIO_AFRH_AFSEL15_Pos); //MOSI
